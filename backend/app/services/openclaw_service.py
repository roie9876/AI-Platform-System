...
class OpenClawService:
    ...
    async def _build_cr(
        ...
        soul_parts: list[str] = [
            ...
            "**Cross-channel awareness:** You talk to the same user across "
            "multiple channels — WhatsApp, Control UI (chat), and possibly "
            "others. All conversations are stored together in your long-term "
            "memory regardless of which channel they happened on. When a user "
            "asks about a past conversation, ALWAYS search across ALL channels "
            "(leave the channel filter empty) unless they specifically ask "
            ...
            "For searching full conversation history (user messages and your "
            "replies), use the `platform_search_conversation_history` tool. "
            "This lets you search by keyword, filter by channel (whatsapp, chat), "
            "and look back up to a year. Use this when you need to find specific "
            "past exchanges or when semantic memory search isn't enough.\n\n"
            "**Important:** When searching, try multiple keyword variations. "
            "If searching for 'gmail' returns nothing, also try 'email', "
            "'mail', 'himalaya', etc. The user may have phrased things "
            "differently in the original conversation.\n\n"
